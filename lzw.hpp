...
// Default error handler for LZW_ERROR().
...
void fatalError(const char * message)
{
    std::cerr << "LZW encoder/decoder error: " << message << std::endl;
    std::abort();
}
...
{
    UByte * newMemory = reinterpret_cast<UByte *>(LZW_MALLOC(bytesWanted));
...
bool BitStreamReader::readNextBit(bool & bitOut)
{
...
static bool outputByte(int code, UByte *& output, int outputSizeBytes, int & bytesDecodedSoFar)
{
